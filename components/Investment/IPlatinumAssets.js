...
const IPlatinumAssets = () => {
  ...
  const uniqueOptions = useMemo(() => {
    return {
      companyType: ["All", ...new Set(platinumAssetsData.map(item => item["Company Type"]).filter(Boolean))],
      country: ["All", ...new Set(platinumAssetsData.map(item => item["Domiciled"]).filter(Boolean))],
      stage: ["All", ...new Set(platinumAssetsData.map(item => item["Company Type"]).filter(Boolean))],
      mineLocation: ["All", ...new Set(platinumAssetsData.map(item => item["Mine Location Country"]).filter(Boolean))],
      primaryResource: ["All", ...new Set(platinumAssetsData.map(item => item["Primary Assets"]).filter(Boolean))],
    };
  }, []);
