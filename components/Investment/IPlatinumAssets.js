...
import { platinumAssetsData } from "@/public/static-data/platinumAssetsData";

const PAGE_SIZE = 15;

const IPlatinumAssets = () => {
  ...
  const uniqueOptions = useMemo(() => {
    return {
      ...
      country: [
        "All",
        ...new Set(
          platinumAssetsData.map((item) => item["Domiciled"]).filter(Boolean),
        ),
      ],
      stage: [
        "All",
      ...
      mineLocation: [
        ...
        ...new Set(
          platinumAssetsData
            .map((item) => item["Mine Location Country"])
            .filter(Boolean),
        ),
      ],
      primaryResource: [
  ...
  const filteredAndSortedData = useMemo(() => {
    let filtered = platinumAssetsData.filter((asset) => {
      const matchesSearch =
        asset["Company Name"]
          ?.toLowerCase()
          .includes(searchTerm.toLowerCase()) ||
        asset["Ticker"]?.toLowerCase().includes(searchTerm.toLowerCase()) ||
        asset["Mine Location Country"]
          ?.toLowerCase()
          .includes(searchTerm.toLowerCase());

      const matchesFilters =
        (filters.companyType === "All" ||
          asset["Company Type"] === filters.companyType) &&
        (filters.country === "All" || asset["Domiciled"] === filters.country) &&
        (filters.stage === "All" || asset["Company Type"] === filters.stage) &&
        (filters.mineLocation === "All" ||
          asset["Mine Location Country"] === filters.mineLocation) &&
        (filters.primaryResource === "All" ||
          asset["Primary Assets"] === filters.primaryResource);
  ...
  return (
    <div className="py-5">
      <h1 className="cambay text-[22px] sm:text-3xl font-semibold">
        Platinum Assets Database
      </h1>

      <div className="mt-1 md:mt-5">
        <div className="w-full bg-[#00AEEF]/10 border border-date/20 p-2 py-4 md:p-8 rounded-lg mb-24">
          <div className="mb-4">
            ...
            <div className="flex gap-x-4 mt-3">
              ...
              <div className="min-w-[120px]">
                ...
                <select
                  ...
                  onChange={(e) =>
                    handleFilterChange("mineLocation", e.target.value)
                  }
                >
              ...
              <div className="min-w-[120px]">
                ...
                <select
                  ...
                  onChange={(e) =>
                    handleFilterChange("primaryResource", e.target.value)
                  }
                >
            ...
            <table className="w-full min-w-[1400px] bg-white border rounded-md">
              <thead className="font-bold border-b">
                <tr>
                  ...
                  >
                    COUNTRIES{" "}
                    {sortColumn === "Mine Location Country" &&
                      (sortDirection === "asc" ? "↑" : "↓")}
                  </th>
                  <th
                    className="px-4 py-[15px] text-left text-[11px] font-semibold uppercase tracking-wider cursor-pointer hover:bg-gray-50"
                  ...
                  >
                    STATE/REGION{" "}
                    {sortColumn === "Mine Location State/Region" &&
                      (sortDirection === "asc" ? "↑" : "↓")}
                  </th>
                  <th
                    className="px-4 py-[15px] text-left text-[11px] font-semibold uppercase tracking-wider cursor-pointer hover:bg-gray-50"
              ...
              <tbody className="bg-bg text-lightgray">
                {paginatedData.map((asset, index) => (
                  ...
                  >
                    ...
                    <td className="px-4 py-[12px] font-semibold text-[#00AEEF]">
                      {asset["Ticker"]}
                    </td>
                    ...
                    <td className="px-4 py-[12px]">
                      <span
                        className={`px-2 py-1 rounded text-xs ${
                          asset["Company Type"] === "Producer"
                            ? "bg-green-100 text-green-800"
                            : asset["Company Type"] === "Developer"
          ...
          <div className="flex justify-between items-center mt-6">
            <div className="text-sm text-gray-600">
              ...
              {Math.min(currentPage * PAGE_SIZE, filteredAndSortedData.length)}{" "}
...
export default IPlatinumAssets;
